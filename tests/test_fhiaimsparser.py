...
def test_scf_spinpol(parser):
    ...
    assert archive.section_run[0].time_run_wall_start.magnitude == pytest.approx(2.23485023e+08)
    ...
    assert sec_atom_type.atom_type_mass.magnitude == pytest.approx(9.27328042e-26)
    assert sec_atom_type.x_fhi_aims_section_controlInOut_atom_species[0].x_fhi_aims_controlInOut_species_cut_pot.magnitude == pytest.approx(3.5e-10)

    assert len(archive.section_run[0].section_system) == 1
    sec_system = archive.section_run[0].section_system[0]
    assert sec_system.lattice_vectors[1][1].magnitude == pytest.approx(-1.4e-10)
    ...
    assert sec_scc.energy_XC_functional.magnitude == pytest.approx(-2.3433685e-16)
    assert sec_scc.single_configuration_calculation_converged
    assert sec_scc.energy_reference_fermi[1].magnitude == pytest.approx(-1.50351795e-18)
    sec_scfs = sec_scc.section_scf_iteration
    assert len(sec_scfs) == 15
    assert sec_scfs[12].energy_total_T0_scf_iteration.magnitude == pytest.approx(-5.56048676e-15)
    assert sec_scfs[5].energy_reference_lowest_unoccupied_iteration[0].magnitude == pytest.approx(-1.42557688e-18)
    assert sec_scfs[7].energy_change_scf_iteration.magnitude == pytest.approx(-1.42557688e-18)
    ...
    assert sec_eig.eigenvalues_values[1][2][4].magnitude == pytest.approx(-1.1221523e-16)
...
def test_geomopt(parser):
    ...
    assert sec_sccs[2].energy_C.magnitude == pytest.approx(-9.35075519e-18)
    assert len(sec_sccs[3].section_scf_iteration) == 6
    assert np.max(sec_sccs[3].atom_forces_free_raw.magnitude) == pytest.approx(2.4933233e-11)
...
def test_band_spinpol(parser):
    ...
    assert sec_k_band.section_k_band_segment[0].band_occupations[1][9][2] == pytest.approx(1.0)
    assert sec_k_band.section_k_band_segment[1].band_energies[0][3][5].magnitude == pytest.approx(-1.39686827e-17)
    assert sec_k_band.section_k_band_segment[2].band_k_points[14][2] == pytest.approx(0.5)

    sec_dos = sec_scc.section_dos[0]
    assert np.shape(sec_dos.dos_energies) == (50,)
    assert np.shape(sec_dos.dos_values) == (2, 50)
    assert sec_dos.dos_energies[46].magnitude == pytest.approx(-2.06321113e-18)
    assert sec_dos.dos_values[0][46] == pytest.approx(1.2418252951253564e-11)
    assert sec_dos.dos_values[1][15] == pytest.approx(3.9151704709731774e-11)
    ...
    assert sec_atom_dos.atom_projected_dos_energies[49].magnitude == pytest.approx(-1.12152364e-18)
    assert sec_atom_dos.atom_projected_dos_values_lm[2][1][0][22] == pytest.approx(6.61986474e+18)
    assert sec_atom_dos.atom_projected_dos_values_lm[0][0][0][8] == pytest.approx(3.35029976e+17)
    assert sec_atom_dos.atom_projected_dos_values_total[0][0][40] == pytest.approx(9.950877863070872e+17)
...
def test_dos(parser):
    ...
    assert sec_species_dos.species_projected_dos_energies[9].magnitude == pytest.approx(-1.35040602e-18)
    assert sec_species_dos.species_projected_dos_values_total[0][0][44] == pytest.approx(3.89674869e+18)
    assert sec_species_dos.species_projected_dos_values_total[0][1][37] == pytest.approx(7.85534487e+17)
    assert sec_species_dos.species_projected_dos_values_lm[1][0][0][3] == pytest.approx(4.49311696e+17)
    assert sec_species_dos.species_projected_dos_values_lm[2][0][1][5] == pytest.approx(2.46401047e+16)
...
def test_md(parser):
    ...
    assert sec_sccs[4].energy_sum_eigenvalues.magnitude == pytest.approx(-1.82257271e-16)

    sec_systems = archive.section_run[0].section_system
    assert len(sec_systems) == 6
    assert sec_systems[2].atom_velocities[1][2].magnitude == pytest.approx(-1195.746888)
    assert sec_systems[4].atom_positions[2][0].magnitude == pytest.approx(2.00130424e-10)
...
def test_gw(parser):
    ...
    assert sec_scfs[1].x_fhi_aims_scgw_galitskii_migdal_total_energy.magnitude == pytest.approx(-1.28528018e-17)
    assert sec_scfs[4].x_fhi_aims_single_particle_energy.magnitude == pytest.approx(-4.96262869e-18)

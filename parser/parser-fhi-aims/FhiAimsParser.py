...
#1.0/27.21138505 # http://physics.nist.gov/cgi-bin/cuu/Convert?exp=0&num=1&From=hr&To=ev&Action=Only+show+factor
from_bohr = 1.0e-10/0.52917721092 # http://physics.nist.gov/cgi-bin/cuu/Value?bohrrada0
from_eV = 1.6021766208e-19
...
sections = [
    RootSection(name='newRun',
        ...
        GroupSection("ProgramHeader",
            subParsers=[
            SLParser(r" *Version +(?P<program_version>[0-9a-zA-Z_.]*)"),
            SLParser(r" *Compiled on +(?P<fhi_aims_program_compilation_date>[0-9/]+) at (?P<fhi_aims_program_compilation_time>[0-9:]+) *on host +(?P<program_compilation_host>[-a-zA-Z0-9._]+)", kind=MatchKind.ExpectFail),
            SkipSome(),
            SLParser(r" *Date *: *(?P<fhi_aims_program_execution_date>[-.0-9/]+) *, *Time *: *(?P<fhi_aims_program_execution_time>[-+0-9.EDed]+)"),
            SLParser(r" *Time zero on CPU 1 *: *(?P<time_run_cpu1_0>[-+0-9.EeDd]+) *s\."),
            SLParser(r" *Internal wall clock time zero *: *(?P<time_run_wall_0>[-+0-9.EeDd]+) *s\."),
        ...
        LineParser("convergedEnergy",
            ...
            SLParser(r"\s*\|\s*Total energy uncorrected\s*:\s*(?P<totalDftEnergy>[-+0-9.eEdD]+) *eV",
                localProps={'totalDftEnergy':PropertyTransformer('totalDftEnergy', lambda x: x*from_Hartree)}),
            SLParser(r"\s*\|\s*Total energy corrected\s*:\s*(?P<totalDftEnergyT0>[-+0-9.eEdD]+) *eV",
                localProps={'totalDftEnergyT0':PropertyTransformer('totalDftEnergyT0',lambda x: x*from_Hartree)}),
        ...
        LineParser("finalEnergies",
            ...
            SLParser(r"\s*\|\s*Total energy of the DFT / Hartree-Fock s.c.f. calculation\s*:\s*(?P<finalTotalDftEnergy>[-+0-9.eEdD]+)\s*eV",
                localProps={'finalTotalDftEnergy':PropertyTransformer('finalTotalDftEnergy', lambda x: x*from_Hartree)}),
            SLParser(r"\s*\|\s*Final zero-broadening corrected energy \(caution - metals only\)\s*:\s*(?P<finalTotalDftEnergyT0>[-+0-9.eEdD]+)\s*eV",
                localProps={'finalTotalDftEnergyT0':PropertyTransformer('finalTotalDftEnergyT0', lambda x: x*from_Hartree)}),
            SLParser(r"\s*\|\s*Total energy after the post-s\.c\.f\. correlation calculation\s*:\s*(?P<finalCorrectedEnergy>[-+0-9.eEdD]+)\s*eV",
                kind=MatchKind.Sequenced|MatchKind.ExpectFail,
                localProps={'finalCorrectedEnergy':PropertyTransformer('finalCorrectedEnergy',lambda x: x*from_Hartree)}),
...
class FhiAimsParser:
    ...
    def __init__(self, backend, normalizer, path):
    ...
    def parseBand(self, path, trajFile):
        ...
        faimsout.close()
        bandSegments = numpy.zeros((Nsegments,2,3),dtype=float)
        #self.datasetForProperty(Property.withName('band_segm_start_end'), (Nsegments,2,3))  # start and end k-point for each segment
        bandLabels   = numpy.zeros((Nsegments,2), dtype=object)
        #self.datasetForProperty(Property.withName('band_segm_labels'), (Nsegments,2))  # start and end label for each segment
        labels = []
        bandstartnames = []
        bandendnames = []
        faimsout=open(path, "r")
            ...
            if "output band" in line:
                if "k_grid" in lines[i-1]:
                    for iband in range(i,i+Nsegments):
                        ...
                        bandSegments[iband-i,0,:] = map(float,wordsbn[2:5])
                        bandSegments[iband-i,1,:] = map(float,wordsbn[5:8])
                    bandLabels[:,0] = bandstartnames
                    bandLabels[:,1] = bandendnames
        self.backend.addArrayValues('band_segm_start_end', bandSegments)
        self.backend.addArrayValues('band_segm_labels', bandLabels)
        ...
        if not os.path.exists(fbandname):
            return None
        ...
        fband.close()
        kPoints = numpy.zeros((Nsegments, nKPoints, 3), dtype = float)
        #self.datasetForProperty(Property.withName('band_k_points'), (Nsegments, nKPoints, 3))
        occupations = zeros((Nsegments, max_spin_channel, nKPoints, nEVs), tdype = float)
        #self.datasetForProperty(Property.withName('band_occupations'), (Nsegments, max_spin_channel, nKPoints, nEVs))
        energies = zeros((Nsegments, max_spin_channel, nKPoints, nEVs), dtype=float)
#self.datasetForProperty(Property.withName('band_energies'), (Nsegments, max_spin_channel, nKPoints, nEVs))
        for iband in range(0,Nsegments):
            for ispin in range(0,max_spin_channel):
                ...
                for line in fband:
                    ...
                    idx += [ int(words[0]) ]
                    iEV = len(idx)-1
                    kPoints[iband, iEV, 0] = float(words[1])
                    kPoints[iband, iEV, 1] = float(words[2])
                    kPoints[iband, iEV, 2] = float(words[3])
                    occupations[iband, ispin, iEV, :] =  map(float,words[4::2])
                    energies[i, iband, ispin, iEV, :] =  map(float,words[5::2])
                fband.close()
        self.backend.addArrayValues('band_k_points', kPoints)
        self.backend.addArrayValues('band_occupations', occupations)
        self.backend.addArrayValues('band_energies', energies)
    def endRun(self,endKind):
        if not self.runDir is None:
            nLog.info("Writing normalized run of %s", self.path)
            self.backend.addValue("run_clean_end", (endKind == CloseKind.Normal))
            ...
            #dataset2[0]=infoStr
            #self.runTraj.close()
            self.backend.closeSection("section_run", self.run_id)
    ...
    def newRun(self):
        self.run_id = self.backend.openSection("section_run")
        if self.runDir:
            self.endRun()
